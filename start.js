...
function patchConfigClaw(tgToken, proxyType = "qwen") {
  ...
  config.models.providers[provName] = {
    ...
    models:  [
      {
        id:            modelId,
        name:          isKimi ? "Kimi K2.6" : isDeepSeek ? (modelId.includes("pro") || modelId.includes("expert") ? "DeepSeek V4 Pro" : "DeepSeek V4 Flash") : "Qwen 3.7 Max",
        reasoning:     false,
        input:         ["text"],
...
function patchHermesConfig(proxyType = "qwen") {
  ...
  const dsModel    = (process.env.DEEPSEEK_MODEL  || "deepseek-v4-flash").replace(/"/g, "");
