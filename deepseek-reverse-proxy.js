...
const TELEMETRY_USER = {
  user_unique_id: "0e14a5a9-8582-43d5-87cd-6f891bc7a682",
  web_id: "7651991877188626947",
};
...
async function sendTelemetry(eventsArray) {
    ...
    const req = https.request({
      ...
      collectBody(res).then(buf => {
        ...
        try {
          ...
          const fileId = biz.id;
          if (!fileId) {
            sendUploadTelemetry({ sessionId: "", fileId: "", fileName: filename, ext: (filename.split(".").pop() || "bin"), fileSize: buffer.length, success: false });
